...
function tableToXlsx(tables, id, cb) {
    var tables = tables instanceof Array? tables : [tables];
    var workbook = excelbuilder.createWorkbook(options.tmpDir, id + ".xlsx");
    for (var xx = 0; xx < tables.length; xx++) {
    var table = tables[xx];
        var name = 'sheet' + (xx + 1);

        var sheet1 = workbook.createSheet(name, getMaxLength(table.rows), table.rows.length);
